...
export class UnlimitClient {
    ...
    static unlimitApi: AxiosInstance = axios.create({
        baseURL: `${EnvUtils.unlimitBaseUrl}/`,
    ...
    async quote(offrampQuoteRequest: OfframpQuoteRequest): Promise<OfframpQuoteResponse> {
        try {
            const response = await UnlimitClient.unlimitApi.get<OfframpQuoteResponse>('offramp/v1/quotes', {
                params: {
                    ...offrampQuoteRequest,
